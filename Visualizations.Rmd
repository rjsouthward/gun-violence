...
map <- get_urbn_map(map = 'territories_states') |>
  ...
  filter(! state_name %in% c("American Samoa", "Guam", "Mariana Islands", "Virgin Islands"))
      ...
      if (input$state == 'United States') {
        map <- map |> mutate(is_colored = factor(if_else(state_name %in% as.list(unique(data$Source_State)), 1, 0)))
      } else {
        map <- map |> mutate(is_colored = factor(if_else(state_name == input$state, 1, 0)))
      }
